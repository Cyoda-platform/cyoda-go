...
// Gt/Lt/Gte/Lte comparison must compare the way the kernel does (byte
// order), not the way the database's own default collation happens to.
//
// query_planner.go's orderingOp builds that WHERE fragment; searcher.go's
// orderByFieldExpr has said COLLATE "C" on the equivalent ORDER BY
// expression from the start, precisely because the kernel
...
// NOT "C" can disagree with that. Before this task, orderingOp carried no
// such collation, so under a non-C database collation a narrowing WHERE
// range could disagree with the kernel and drop a row the kernel would
// have matched. That under-select is unrecoverable: a row the WHERE
// excludes is never fetched from the database, so the Go-side postFilter
// re-check (see soundness_property_test.go's doc comment) never gets a
// chance to save it.
//
...
func TestPostgresPushdownSoundness_TextRangeUnderNonCCollation(t *testing.T) {
	...
	// Assertion 1: the raw SQL candidate set (the WHERE fragment orderingOp
	// builds, before any Go-side postFilter re-check) must be a superset of
	// the kernel's true matches. Before COLLATE "C", the database's ICU
	// default collation sorts e-lower's value BEFORE e-upper's, so the
	// pushed >= comparison excludes e-lower — an under-select the re-check
	// stage can never recover, because the row is never fetched.
	candidateIDs, err := postgres.SearchCandidateIDsForTest(pool, fctx, tenant, gsModel.EntityName, gsModel.ModelVersion, filter)
	if err != nil {
	...
	// Assertion 2: the full Search() pipeline (WHERE + postFilter re-check)
	// must equal the kernel's true matches exactly.
	results, err := store.Search(fctx, filter, spi.SearchOptions{ModelName: gsModel.EntityName, ModelVersion: gsModel.ModelVersion, Limit: 10})
	if err != nil {
