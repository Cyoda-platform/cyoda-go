...
// transactionId from the first StateMachine event (the workflow engine generates
// its own transaction ID, which differs from the entity-level transaction ID).
...
func TestGetStateMachineFinishedEvent_Found(t *testing.T) {
	...
	// The SM transaction ID is generated by the workflow engine, not the entity handler.
