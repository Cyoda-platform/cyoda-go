...
## Supersession notice — bootstrap credential semantics (added 2026-04-17 during Task 2 implementation)

The `CYODA_BOOTSTRAP_CLIENT_ID` / `CYODA_BOOTSTRAP_CLIENT_SECRET` validation and chart behavior were sharpened after a factual investigation of `app/app.go:186` call-site behavior. Landed in binary commit `193dd48`:

- **Coupled predicate in jwt mode:** ID and Secret must both be set, or both be empty. Half-configured states are rejected with an error that names both vars. (Earlier draft of Task 2 said "jwt requires secret always" — that was stricter than the call site.)
- **Chart: bootstrap is opt-in.** `bootstrap.clientId` defaults to `""`. The chart renders the bootstrap Secret (either chart-managed auto-gen OR operator existingSecret) AND wires `CYODA_BOOTSTRAP_CLIENT_SECRET_FILE` + the projected-volume item **only** when `bootstrap.clientId` is non-empty.

**Implications for downstream tasks:**

- **Task 8 (chart-managed Secrets):** `secret-bootstrap.yaml` is wrapped in `{{- if .Values.bootstrap.clientId }}…{{- end }}` in addition to the existing `if not .Values.bootstrap.clientSecret.existingSecret` gate. Both conditions must be true for the chart to render a bootstrap Secret.
- **Task 9 (StatefulSet):** the `CYODA_BOOTSTRAP_CLIENT_SECRET_FILE` env entry AND the projected-volume `bootstrap-client-secret` item are wrapped in `{{- if .Values.bootstrap.clientId }}…{{- end }}`. When `bootstrap.clientId` is empty, the pod has no bootstrap-secret env and no bootstrap-secret file — consistent with the binary's "both empty = legitimate" path.
- **Any `helm template` / `helm install` invocation using `--set bootstrap.clientSecret.existingSecret=...`** must also pass `--set bootstrap.clientId=<some-id>` (e.g. `test-bootstrap`) to exercise the bootstrap path. Otherwise the chart correctly elides the bootstrap Secret and volume item, and the `--set` for `existingSecret` is simply ignored.
- **Task 2 binary code** has already landed the coupled-predicate validator (`193dd48`). The plan text for Task 2 below describes the earlier draft; the implementation supersedes it.
...
bootstrap:
  # -- Bootstrap M2M client is OPT-IN. When clientId is empty, the
  # chart renders no bootstrap Secret and the pod runs with no
  # bootstrap M2M credential — the binary starts cleanly in jwt mode
  # via its coupled predicate (both-empty is legitimate). Set clientId
  # to enable bootstrap; the chart will auto-generate the secret via
  # the lookup+GitOps-safety pattern, or use clientSecret.existingSecret
  # if operator-provided.
  clientId: ""
  clientSecret:
    existingSecret: ""
    existingSecretKey: secret
