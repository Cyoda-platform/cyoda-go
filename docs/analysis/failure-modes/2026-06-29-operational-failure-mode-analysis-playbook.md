...
| F8 | `ReadHeaderTimeout` (10s), `ReadTimeout` (5m) and `IdleTimeout` (120s) are set on both servers from `CYODA_HTTP_*`; `WriteTimeout` ships `0` and there is deliberately no per-request deadline — the server imposes no time budget on work (search bounding contract). Pool acquire is bounded separately (F1). | `cmd/cyoda/httpserver.go`; `app/config.go` (`HTTPConfig`) | **Closed.** Regressed if a server is built outside `newHTTPServer` or a timeout is set to `0` |
...
| F10 | The gRPC server DOES have recovery interceptors, unary and stream, chained first so they also cover a panic inside auth or tx-routing, and they latch the node unhealthy; every write to a member's stream goes through the member's outbox (`Member.Send` or `Member.TrySend`), so all of them are written by that member's single writer goroutine; a write stalled longer than the keep-alive timeout evicts the member — the greet included, which matters because `Register` publishes the member before the greet is written and a dispatch routed in that window would otherwise interleave HTTP/2 frames | `internal/grpc/server.go` (`UnaryRecoveryInterceptor`, `StreamRecoveryInterceptor`); `internal/grpc/streaming.go` (greet handed to `Register` as the writer's first event; keep-alive via `member.TrySend`); `members.go` (`Member.Send`, `Member.TrySend`, `writeLoop`, `MemberRegistry.Register`); `single_writer_exit_check_test.go` (`TestSingleWriter_OnlyOneRawSendOnTheMemberStream`, a `go/ast` exit check) | **Closed.** Regressed if a panic no longer latches the node unhealthy, or if any code outside `writeLoop` calls the raw `stream.Send` on the member stream — pinned by `TestSingleWriter_OnlyOneRawSendOnTheMemberStream` |
...
1. Keep each run's machine‑readable ledger (the workflow's `data` array → `verified-findings.json`). The current baseline is the [Appendix table](./2026-06-29-operational-failure-mode-analysis.md#appendix--full-finding-ledger) (54 findings; 24 confirmed / 22 partial / 6 closed / 1 decided / 1 refuted).
