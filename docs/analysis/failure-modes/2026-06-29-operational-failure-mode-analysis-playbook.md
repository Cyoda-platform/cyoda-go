...
| F10 | The gRPC server DOES have recovery interceptors, unary and stream, chained first so they also cover a panic inside auth or tx-routing, and they latch the node unhealthy; every write to a member's stream goes through the member's outbox (`Member.Send` or `Member.TrySend`), so all of them are written by that member's single writer goroutine; a write stalled longer than the keep-alive timeout evicts the member — the greet included, which is the writer's first item and is queued before the member is published, so no dispatch can precede it | `internal/grpc/server.go` (`UnaryRecoveryInterceptor`, `StreamRecoveryInterceptor`); `internal/grpc/streaming.go` (greet handed to `Register` as the writer's first event; keep-alive via `member.TrySend`); `members.go` (`Member.Send`, `Member.TrySend`, `writeLoop`, `MemberRegistry.Register`); `single_writer_exit_check_test.go` (`TestSingleWriter_OnlyOneRawSendOnTheMemberStream`, a `go/ast` exit check) | **Closed.** Regressed if a panic no longer latches the node unhealthy, or if any code outside `writeLoop` calls the raw `stream.Send` on the member stream — pinned by `TestSingleWriter_OnlyOneRawSendOnTheMemberStream` |
