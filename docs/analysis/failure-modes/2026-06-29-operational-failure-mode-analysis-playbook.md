...
| F10 | The gRPC server DOES have recovery interceptors, unary and stream, chained first so they also cover a panic inside auth or tx-routing, and they latch the node unhealthy; every send to a member's stream goes through `Member.Send`, so all of them are written by that member's single writer goroutine; a write stalled longer than the keep-alive timeout evicts the member — the greet included, which matters because `Register` publishes the member before the greet is written and a dispatch routed in that window would otherwise interleave HTTP/2 frames | `internal/grpc/server.go` (`UnaryRecoveryInterceptor`, `StreamRecoveryInterceptor`); `internal/grpc/streaming.go` (greet and keep-alive both via `member.Send`); `members.go` (`Member.Send`, `MemberRegistry.Get`) | **Closed.** Regressed if a panic no longer latches the node unhealthy, or if any new write to a member's stream calls `stream.Send` directly instead of `Member.Send` |
