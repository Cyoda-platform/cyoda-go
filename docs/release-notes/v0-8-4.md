...
One grammar, one resolver, one answer — and a server that stays up while it
gives it.
...
[breaking changes](#️-breaking-changes--behavioral-updates) before upgrading.
...
`$.city` used to be looked up as `$.$.city`. See
...
v0.8.4 is a patch release that carries breaking changes — listed under
...
release departs from that on the path grammar, the search execution contract,
the model's type-admission rules, and the SPI. Read the breaking-changes list
before upgrading rather than inferring compatibility from the version number.
