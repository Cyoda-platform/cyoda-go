...
| `numeric-type-admission.md` | A field holds a value when its declared type admits it — a per-value predicate, never a range check or a classified-label check; the `DOUBLE` precision-and-scale conjunct and why range alone reintroduces an unfindable value; a text-declared field no longer learns a temporal subtype from a write; `EQUALS`/`NOT_EQUAL` corrections on trailing-zero and negative-zero operands; the accepted order-dependence for concurrent numeric/temporal-leaf folding and the monotone-and-admits-every-written-value property that replaces byte-identical convergence there; the obligation on a backend that indexes by declared type |
